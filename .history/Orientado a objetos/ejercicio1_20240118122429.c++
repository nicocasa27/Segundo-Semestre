...
if (longg < -2147483648 || longg > 2147483647) {
    ...
    while (!(cin >> floatt)) {
        cin.clear();
        cin.ignore();
        cout << "Entrada no válida. Introduce un float válido: ";
    }

    if (floatt < 3.4E-38 || floatt > 3.4E38) {
        cout << "El dato está fuera de rango." << endl;
        return 1;
    }
...
while (!(cin >> doublee)) {
    ...
    if (!std::isalpha(charr)) {
        std::cerr << "No se puede tener mas numeros " << endl;
        return 1;
    }
